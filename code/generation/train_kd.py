...
from model.trainer_kd import Trainer
...
def main():
    ...
    transformer_T = TransformerModel(n_layers=model_config.n_layers,
                                   n_embeddings=len(vocab),
                                   n_pos_embeddings=model_config.n_pos_embeddings,
                                   embeddings_size=model_config.embeddings_size,
                                   padding_idx=vocab.pad_id,
    ...
    state_dict = torch.load(trainer_config.teacher_checkpoint_path, map_location=device)
    ...
    transformer_T.load_state_dict(temp, strict=True)
    print('load teacher model ok')
    ...
    if distributed:
        ...
        model_trainer.model_T.transformer_module = DistributedDataParallel(model_trainer.model_T.transformer_module,
                                                                         device_ids=[args.local_rank], output_device=args.local_rank)
    ...
    def save_func(epoch):
        ...
        if os.path.exists(trainer_config.last_checkpoint_path + str(epoch - 150)):
            os.remove(trainer_config.last_checkpoint_path + str(epoch - 150))
