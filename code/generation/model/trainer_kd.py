...
import logging
import math
logger = logging.getLogger('s2s-dialog-fake-kd')
logger.setLevel(logging.INFO)
fh = logging.FileHandler('s2s-dialog-fake-kd.log', encoding='utf-8')
...
class Trainer:
    def __init__(self, model, model_T, train_dataset, test_dataset=None, batch_size=8,
        ...
        self.model_T.requires_grad = False
        # self.model_T.eval()
        ...
        self.kl_loss = torch.nn.KLDivLoss(reduction='batchmean').to(device)
        #base_optimizer = Adam(self.model.parameters(), lr=lr, weight_decay=0.01)
        base_optimizer = Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=lr, weight_decay=0.01)
        self.optimizer = NoamOpt(self.model.embeddings_size, 0.05, lr_warmup, base_optimizer)
        ...
        self.train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=batch_size//batch_split,
                                           shuffle=(not distributed), num_workers=n_jobs, collate_fn=self.collate_func_cn)
    ...
    def collate_func_cn(self, data):
        x, y = zip(*data)
        contexts = []

        if max(map(len, x)) > 0:
            x = [torch.tensor(d, dtype=torch.long) for d in x]
    ...
    def _eval_train(self, epoch, risk_func=None):
        ...
        for i, (contexts, targets) in enumerate(tqdm_data):
            ...
            for context in contexts:
                ...
                enc_context_t = self.model_T.encode(context.clone())
                enc_contexts.append(enc_context)
                enc_contexts_t.append(enc_context_t)
                # print('enc_context: ', enc_context)
                # print('enc_context_t: ', enc_context_t)
            ...
            outputs_t = self.model_T.decode(prevs, enc_contexts_t)
            outputs_t = F.softmax(outputs_t, dim=-1)
            batch_kl_loss = self.kl_loss(outputs.view(-1, outputs.shape[-1]), outputs_t.view(-1, outputs_t.shape[-1]))
            '''
            # teacher model
            prevs_t = targets[:, :-1].contiguous()
            enc_contexts_t = []
            for context in contexts:
                temp = context.clone()
                # print('temp: ', temp)
                enc_context_t = self.model_T.encode(temp)
                enc_contexts_t.append(enc_context_t)
                print('enc_context_t: ', enc_context_t)
            outputs_t = self.model_T.decode(prevs_t, enc_contexts_t)
            # print('outputs_t: ', outputs_t)
            outputs_t = F.softmax(outputs_t, dim=-1)
            batch_kl_loss = self.kl_loss(outputs.view(-1, outputs.shape[-1]), outputs_t.view(-1, outputs_t.shape[-1]).data)
            print(outputs.shape, outputs_t.shape, batch_kl_loss)
            '''

            # optimization
            full_loss = (batch_lm_loss * self.lm_weight + batch_loss + batch_kl_loss) / self.batch_split
            full_loss.backward()
            ...
            kl_loss = (i * kl_loss + batch_kl_loss.item()) / (i + 1)
            tqdm_data.set_postfix({'lm_loss': lm_loss, 'loss': loss, 'kl_loss': kl_loss, 'ppl': math.exp(loss), 'loss_step': batch_loss.item(),
                 'lr': self.optimizer.rate(), 'step': self.optimizer._step})

        log_dict = {'epoch': epoch, 'lm_loss': lm_loss, 'loss': loss, 'kl_loss': kl_loss, 'ppl': math.exp(loss), 'lr': self.optimizer.rate(),
    ...
    def _eval_test(self, metric_funcs={}):
        ...
        for i, (contexts, targets) in enumerate(tqdm_data):
            ...
            tqdm_data.set_postfix(dict({'lm_loss': lm_loss, 'loss': loss, 'ppl': math.exp(loss)}, **metrics))
        log_dict = dict({'lm_loss': lm_loss, 'loss': loss, 'ppl': math.exp(loss)}, **metrics)
        log_dict_json = json.dumps(log_dict, ensure_ascii=False)
        logger.info(log_dict_json)
