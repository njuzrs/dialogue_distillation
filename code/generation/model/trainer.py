...
fh = logging.FileHandler('s2s.log', encoding='utf-8')
...
class Trainer:
    ...
    def _eval_train(self, epoch, risk_func=None):
        ...
        for i, (contexts, targets) in enumerate(tqdm_data):
            ...
            tqdm_data.set_postfix({'lm_loss': lm_loss, 'loss': loss, 'ppl': math.exp(loss), 'loss_step': batch_loss.item(),
                 'lr': self.optimizer.rate(), 'step': self.optimizer._step})

        log_dict = {'epoch': epoch, 'lm_loss': lm_loss, 'loss': loss, 'ppl': math.exp(loss), 'lr': self.optimizer.rate(),
