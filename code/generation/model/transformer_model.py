...
class TransformerModel(nn.Module):
    def __init__(self, n_layers, n_embeddings, n_pos_embeddings, embeddings_size, 
                 ...
                 length_penalty=0.8, annealing_topk=None, temperature=1.0, annealing=0,
    ...
    def predict_beams(self, contexts=[]):
        enc_contexts = [self.encode(c) for c in contexts]
    ...
    def beam_search(self, enc_contexts=[], return_beams=False):
        with torch.no_grad():
            ...
            for i in range(self.max_seq_len):
                ...
                '''
                # zrs
                prevs_list_temp = prevs.tolist()
                for idx in range(batch_size * self.beam_size):
                    b_list = prevs_list_temp[idx]
                    if len(b_list) > 1 and b_list[-1] != self.padding_idx and b_list[-1] != self.eos_id:
                        key = (int(b_list[-2]), int(b_list[-1]))
                        if key in repeat[idx]:
                            for value in repeat[idx][key]:
                                log_probs[idx][value] = -1000
                # **********
